...
class EmployeeRecurringComponent(AuditedModel, TenantScopedModel):
    ...
    **s34 states no ceiling on deductions in total** (D-301, correcting this
    ...
    def clean(self):
        ...
        # D-198, amended. The accommodation deduction may not exceed the cap the
        # instrument in force states for the employer's sector on the line's start
        # date. Read through statutory.resolve, never a literal, and as an explicit
        # THREE-state answer: a percentage of zero used to stand in for "no cap",
        # which made a genuine zero cap unrepresentable and read it as unlimited.
        # clean() rather than a trigger: the cap resolves per sector and date with
        # the BCEA fallback, and a second copy of that resolver in SQL would drift
        # (guarded by employees/tests/test_no_importer_bypasses_clean.py).
        if (
            component.is_system
            ...
            cap = resolve.accommodation_cap(self.employee.employer.sector, self.effective_from)
            if cap.state is AccommodationCapState.NOT_LOADED:
                # D-101's shape: a capture-time check has no staleness guard behind
                # it, so a missing figure refuses rather than waving the line through.
                raise ValidationError(
                    {
                        "percentage_of_basic": (
                            f"The accommodation deduction ceiling cannot be checked: "
                            f"{cap.detail} Load the statutory reference data before "
                            "capturing this line."
                        )
                    }
                )
            if cap.exceeded_by(self.percentage_of_basic):
                raise ValidationError(
                    {
                        "percentage_of_basic": (
                            f"An accommodation deduction of "
                            f"{self.percentage_of_basic:.2f}% exceeds the ceiling of "
                            f"{cap.percentage:.2f}% of the wage in force on "
                            f"{self.effective_from:%d %B %Y} "
                            f"(working_time_rule_set.accommodation_deduction_max_pct, "
                            f"{cap.source_reference})."
                        )
                    }
                )
